<?php
/**
 * Default model for Finna Solr records.
 *
 * PHP version 5
 *
...
{
    /**
     * Return an array of parameters to send to Finna's internal 
     * cover generator.
     *
    ...
    {        
        if ($index == 0) {
            ...
            $urls = $this->getAllThumbnails($size);
    ...
    {
        if (isset($this->mainConfig['ImageRights'])
            && isset($this->mainConfig['ImageRights'][$language])
            && isset($this->mainConfig['ImageRights'][$language][$copyright])
        ) {
